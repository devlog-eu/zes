 (function() {
     ...
     window.trx_addons_set_cookie = function(name, value) {
         ...
         document.cookie = name + '=' + escape(value) + ((expires) ? ';expires=' + expires_date.toGMTString() : '') + ((path) ? ';path=' + path : '') + ((domain) ? ';domain=' + domain : '') + ((secure) ? ';secure' : '') + (samesite ? ';samesite=' + samesite : '');
     };
     ...
     function trx_addons_sc_fullheight(e, container) {
         ...
         container.find('.trx_addons_stretch_height').each(function() {
             ...
             if (fullheight_row.length > 0) {
                 ...
                 var wh = jQuery(window).height() >= 698 ? jQuery(window).height() - trx_addons_fixed_rows_height() : 'auto';
                 if (wh > 0) {
     ...
     function trx_addons_init_sliders(e, container) {
         ...
         if (container.find('.slider_swiper:not(.inited)').length > 0) {
             container.find('.slider_swiper:not(.inited)').each(function() {
                 ...
                 if ((!loaded || init_attempts < 5) && init_attempts < 20) {
                     init_attempts++;
                     setTimeout(function() {
